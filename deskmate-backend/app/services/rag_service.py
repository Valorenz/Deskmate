...
class RAGService:
    ...
    async def answer(
        ...
        if not active_collections:
            logger.warning("⚠️ Tidak ada dokumen yang terindeks di ChromaDB. Pastikan dokumen sudah diupload dan indexing berhasil.")
        ...
        if active_collections:
            retrieved_chunks = await vector_service.search(
                ...
                top_k=top_k,
                score_threshold=0.3,   # Buang chunk dengan relevansi < 30%
            )
            logger.info(f"Retrieved {len(retrieved_chunks)} relevant chunks")
            for i, chunk in enumerate(retrieved_chunks):
                logger.info(f"  Chunk {i+1}: [{chunk.title}] score={chunk.score:.4f} | {chunk.content[:80]}...")
