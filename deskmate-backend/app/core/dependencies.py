...
async def get_current_user(
    ...
    # --- Cek 3: Ambil role asli dari database ---
    try:
        result = await db.execute(select(Profile).where(Profile.id == uuid.UUID(token_payload.sub)))
        profile = result.scalar_one_or_none()
        if profile and profile.role:
            token_payload.role = profile.role
    except Exception as e:
        # Jika gagal akses DB, biarkan role bawaan token (meskipun biasanya 'authenticated')
        pass
