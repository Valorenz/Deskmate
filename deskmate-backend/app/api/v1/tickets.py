...
async def _attach_files_to_ticket(ticket: Ticket, db: AsyncSession):
    from app.models.attachment import Attachment
    from app.core.config import settings
    att_res = await db.execute(
        select(Attachment).where(
            Attachment.context_type == "ticket",
            Attachment.context_id == ticket.id
        )
    )
...
async def create_ticket(body: TicketCreate, background_tasks: BackgroundTasks, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
    ...
    await _attach_files_to_ticket(ticket, db)
