...
export default function DocumentManagementPage() {
  ...
  const [editForm, setEditForm] = useState({ title: "", category: "SOP", description: "" });
  ...
  async function handleReindexDocument(docId, docTitle) {
    if (!confirm(`Apakah Anda yakin ingin melakukan re-index pada dokumen "${docTitle}"?\n\nTindakan ini akan menghapus data vektor di ChromaDB dan membuat ulang indeks dari file di Supabase Storage.`)) {
      return;
    }
    setReindexingId(docId);
    try {
      const r = await apiFetch(`/api/v1/documents/${docId}/reindex`, {
        method: "POST"
      });
      if (r?.ok) {
        alert(`Berhasil: Re-indexing untuk "${docTitle}" telah dijadwalkan di background.`);
        loadDocuments();
      } else {
        const d = await r?.json();
        alert(d?.detail || "Gagal melakukan re-index.");
      }
    } catch (err) {
      console.error(err);
      alert("Terjadi kesalahan koneksi saat melakukan re-index.");
    } finally {
  ...
  async function handleSaveMetadata() {
    ...
    setSavingMetadata(true); setEditError("");
    try {
      const r = await apiFetch(`/api/v1/documents/${selected.id}`, {
        method: "PUT",
      ...
      if (r?.ok) {
        ...
        const d = await r?.json();
        setEditError(d?.detail || "Gagal menyimpan metadata.");
      }
    } catch (err) {
      console.error(err);
      setEditError("Terjadi kesalahan koneksi saat menyimpan metadata.");
    } finally {
  ...
  async function handleReindexAllDocuments() {
    if (!confirm("Apakah Anda yakin ingin melakukan re-index pada SEMUA dokumen?\n\nTindakan ini akan mengunduh ulang setiap file dokumen dari Supabase Storage dan membuat ulang seluruh indeks vektor di ChromaDB. Proses ini dapat memakan waktu beberapa menit.")) {
      return;
    }
    ...
    for (const doc of documents) {
      try {
        const r = await apiFetch(`/api/v1/documents/${doc.id}/reindex`, {
          method: "POST"
        });
        if (r?.ok) {
          successCount++;
    ...
    alert(`Proses Re-index Selesai!\n\n- Berhasil dijadwalkan: ${successCount} dokumen\n- Gagal: ${failCount} dokumen\n\nIndeks sedang diperbarui di latar belakang.`);
    loadDocuments();
  }
  ...
  return (
    <div className="flex h-screen w-full bg-[#f4f6fa] font-sans text-[#111827] overflow-hidden relative">
          ...
          <div className="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4 shrink-0">
            ...
            <div className="flex items-center gap-2 select-none">
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white px-4 py-2 rounded-xl text-xs md:text-sm font-bold shadow-sm transition flex items-center gap-2"
              >
                ...
                {reindexingAll ? "Re-indexing All..." : "Reindex All"}
              </button>
              <button
                onClick={() => setShowUploadModal(true)}
                className="bg-[#003399] hover:bg-[#124090] text-white px-4 py-2 rounded-xl text-xs md:text-sm font-bold shadow-sm transition flex items-center gap-2"
              >
                <svg className="h-4 w-4 text-white shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            ...
            {selected && (
              <div className="w-[300px] md:w-80 border-l border-gray-200 bg-white p-5 md:p-6 overflow-y-auto custom-scrollbar shrink-0 flex flex-col gap-6">
                ...
                <div className="space-y-3 pt-4 border-t border-slate-100 select-none">
                  <button 
                    onClick={() => {
                      setEditForm({
                        ...
                        category: selected.category || "SOP",
                        description: selected.description || ""
                      });
                      setEditError("");
                      setShowEditModal(true);
                    }}
                    className="w-full bg-[#003399] hover:bg-[#124090] text-white py-2 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-sm"
                  >
                  ...
                  <button
                    onClick={() => handleReindexDocument(selected.id, selected.title)}
                    disabled={reindexingId === selected.id}
                    className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white py-2 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-sm"
                  >
                    <svg className="h-3.5 w-3.5 text-white shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.21 7.89H18" />
                    </svg>
                    {reindexingId === selected.id ? "Re-indexing..." : "Reindex Document"}
                  </button>
      ...
      {/* ── EDIT METADATA MODAL ── */}
      {showEditModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 animate-fade-in" onClick={() => setShowEditModal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-slide-up" onClick={e => e.stopPropagation()}>
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
              <h2 className="text-base md:text-lg font-black text-[#111827]">Edit Document Metadata</h2>
              <button onClick={() => setShowEditModal(false)} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-[#111827] transition-colors">
                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
