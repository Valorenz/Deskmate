      ...
      if (res?.ok) {
        ...
        if (attachments.length > 0) {
          for (const file of attachments) {
            ...
            try {
              const uploadRes = await apiFetch(`/api/v1/tickets/${ticket.id}/attachments`, {
                method: "POST",
                body: formData,
              });
              if (!uploadRes || !uploadRes.ok) {
                console.error("Gagal mengunggah salah satu lampiran:", file.name);
              }
            } catch (err) {
              console.error("Error mengunggah berkas:", file.name, err);
            }
          }
        }
        ...
        if (Array.isArray(errDetail)) {
          const messages = errDetail.map(e => {
            let field = e.loc?.[e.loc.length - 1] || "Field";
            if (field === "title") field = "Subject";
            if (field === "description") field = "Description";
            
            if (e.msg && e.msg.startsWith("String ")) {
              return e.msg.replace("String", field);
            }
            return `${field}: ${e.msg}`;
          });
          setError(messages.join(", "));
        } else if (typeof errDetail === "object" && errDetail !== null) {
          setError(JSON.stringify(errDetail));
        } else {
          setError(errDetail || "Gagal membuat tiket. Coba lagi.");
        }
      }
    } catch {
