                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {ticket.attachments.map((att, i) => (
                                <a 
                                  ...
                                  className="flex items-center gap-3 border border-gray-200/80 rounded-xl p-3 bg-slate-50 hover:bg-slate-100/70 transition-colors cursor-pointer"
                                >
                                  <svg className="h-5 w-5 text-gray-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                                  </svg>
                                  <div className="min-w-0 flex-1 text-left">
                                    <p className="text-xs font-bold text-[#003399] truncate hover:underline">{att.file_name}</p>
                                    <span className="text-[10px] text-[#6b7280] block">
                                      {att.file_type} ({att.file_size_bytes ? (att.file_size_bytes / 1024).toFixed(1) : "0"} KB)
                                    </span>
                                  </div>
                                </a>
                              ))}
